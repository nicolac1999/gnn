...
from common.model_helpers import readout_node_values_as_slice, readout_context_values_as_slice, \
    readout_edge_values_as_slice
...
class ResHeatSimplifiedModel(NamedTuple):
    ...
    cell2liquid__kapitza_induced_heat_flux: np.array


# %%

class HeatSimplifiedModel_kapitza_read(keras.Model):
    """
    ...
    def __init__(self, configuration_file, num_steps=30):
        ...
        super(HeatSimplifiedModel_kapitza_read, self).__init__(inputs=in_out['inputs'], outputs=in_out['outputs'])
    ...
    def _create_model(self, num_steps, include_initial_step=True):
        ...
        if include_initial_step:
            ...
            res_kapitza_heat_flux.append(tf.expand_dims(tf.zeros(21), axis=0))
        ...
        for i in range(num_steps):
            ...
            res_heat_flux_conduction.append(readout_edge_values_as_slice(graph, 'conduction', 'heat_flux_conduction'))
            res_delta_t_conduction.append(readout_edge_values_as_slice(graph, 'conduction', 'delta_t_conduction'))
            res_kapitza_heat_flux.append(readout_edge_values_as_slice(graph, 'cell2liquid', 'kapitza_induced_heat_flux_density'))
        ...
        stack_of_kapitza_heat_flux = keras.layers.Concatenate(axis=0)(res_kapitza_heat_flux)
