...
def create_standard_cell(temperatures,
                         ...
                         kapitza_conductance=get_kapitza_conductance()
                         ):
    NUM_CELLS = 20
    NUM_LIQUID = 20
    NUM_HEATERS = 8
    ...
    if masses:
        ...
        masses = np.array([36.,
                           ...
                           21.75, 14.5, 21.75,
                           21.75, 14.5, 21.75,
                           21.75, 14.5, 21.75], dtype=np.float32)
    ...
    edges_cells2cells_heat_flux_conduction = np.ones(38, dtype=np.float32)
    edges_cells2cells_delta_t = np.zeros(38, dtype=np.float32)
    edges_cells2cells_thermal_conductivity = np.ones(38, dtype=np.float32) * liquid_thermal_conductivity
    edges_cells2liquid_kapitza_conductance = np.ones(20, dtype=np.float32) * kapitza_conductance
    edges_liquid2cells_kapitza_conductance = np.ones(20, dtype=np.float32) * kapitza_conductance
