...
def create_standard_cell_with_dynamic_BHX_virtual_pot(temperatures,
                         num_nodes_BHX,
    ...
    if heat_coming_from_adjacent_cell:
        NUM_HEATERS = 9
        HEAT_SUPPLIED_TARGET = [0, 3, 8, 13, 18, 21, 26, 31, 35]
        HEAT_SUPPLIED_SOURCE = [0, 1, 2, 3, 4, 5, 6, 7, 8]

    else:
        NUM_HEATERS = 8
        HEAT_SUPPLIED_TARGET = [0, 3, 8, 13, 18, 21, 26, 31]
        HEAT_SUPPLIED_SOURCE = [0, 1, 2, 3, 4, 5, 6, 7]
    ...
    if power_supplied is not None:
        if len(power_supplied) != NUM_HEATERS:
            raise ValueError('Number of the heaters power supplied not compatible with the'
                             'number of heaters in the standard cell = 8 ,+ 1 heater which '
    ...
    if liquid_flow_direction == 0:
        ...
        cell_node_order_adj_matrix = [x for x in range(NUM_CELLS - 1, NUM_CELLS - int(num_nodes_BHX / num_liquid_nodes_per_cell) - 1, -1)]
        cell_node_order_adj_matrix = np.repeat(cell_node_order_adj_matrix, num_liquid_nodes_per_cell)
        lengths = LENGTH_CELLS[-int(num_nodes_BHX/num_liquid_nodes_per_cell):] / num_liquid_nodes_per_cell
        lengths = np.repeat(lengths, num_liquid_nodes_per_cell)
        lengths = np.concatenate([lengths, LENGHT_POT])
...
#
# g0 = create_standard_cell_with_dynamic_BHX_virtual_pot(temperatures=INITIAL_TEMPERATURES_g1,
#                                                        num_nodes_BHX=5,
#                                                        liquid_flow_direction=1,
