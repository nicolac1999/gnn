...
def half_cell(temperatures,
    ...
    if heat_coming_from_adjacent_cell:
        NUM_HEATERS = 5
        HEAT_SUPPLIED_TARGET = [0, 3, 8, 13, 17]
        HEAT_SUPPLIED_SOURCE = [0, 1, 2, 3, 4]

    else:
        NUM_HEATERS = 4
        HEAT_SUPPLIED_TARGET = [0, 3, 8, 13]
        HEAT_SUPPLIED_SOURCE = [0, 1, 2, 3]
    ...
    LENGTH_CELLS = np.array([30., 440., 30.,
                             30., 310., 310., 310., 30.,
                             30., 310., 310., 310., 30.,
                             30., 310., 310., 310., 30.], dtype=np.float32) * shrinking_factor
    ...
    if masses:
        ...
        masses = np.array([7.7, 15.6, 7.7,
                           7.7, 11.4, 11.4, 11.4, 7.7,
                           7.7, 11.4, 11.4, 11.4, 7.7,
                           7.7, 11.4, 11.4, 11.4, 7.7,], dtype=np.float32) * shrinking_factor
...
# # %% Testing
#
# INITIAL_TEMPERATURES = np.ones(18, dtype=np.float32) * 1.888
#
# half_cell_graph = half_cell(temperatures=INITIAL_TEMPERATURES,
#                                num_nodes_BHX=15,
#                                num_liquid_nodes_per_cell=3,
#                                heat_coming_from_adjacent_cell=False)
#
# # %%
#
# plot_graph_tensor(half_cell_graph)